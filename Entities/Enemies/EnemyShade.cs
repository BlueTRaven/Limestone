...
namespace Limestone.Entities.Enemies
{
    public class EnemyShade : Enemy
    {
        ...
        public EnemyShade(Vector2 position) : base()
        {
            ...
            if (distance < 192)
            {
                if (shot0 <= 0)
                {
                    ...
                    for (float i = -rand; i <= rand; i++)
                    {
                        ...
                        Projectile2 p = new Projectile2(Assets.GetTexFromSource("projectilesFull", 8, 0), new Color(77, 58, 84), 4.5f, position, Vector2.Zero, new Vector2(4, 8), rotToPlayer + (i * 10), 135, 3.25f, 128, 40);
                        world.CreateProjectile(p);

                        p = new Projectile2(Assets.GetTexFromSource("projectilesFull", 0, 1), new Color(77, 58, 84), 4.5f, position, Vector2.Zero, new Vector2(4, 8), rotToPlayer + (i * 10), 135, 3.25f, 128, 40);
                        world.CreateProjectile(p);
                    }
                    prevcount = rand;
                    shot0 = 60;
