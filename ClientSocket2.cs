...
namespace Limestone
{
    ...
    public class ClientSocket2
    {
        ...
        public static void Send(Socket client, String data)
        {
            ...
            List<byte> bytes = new List<byte>(byteData.Length);

            bytes.Add((byte)byteData.Length);   //set the first byte to be the length of the data.
            for (int b = 0; b < byteData.Length; b++)   //This list excludes the header (length of packet)
            { 
                bytes.Add(byteData[b]);   //then add the other stuff back on
            }
